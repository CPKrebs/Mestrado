...
///
float* copy(float* A, int LDA, int M, int N,
            float* B, bool trans, float scalar, bool nullable)
{
  float* result = NULL;
  ...
  if (trans == false && scalar != 1.0)
  {
    for (int i = 0; i < M; ++i)
      for (int j = 0; j < N; ++j)
        result[i * M + j] = A[i + j * LDA] * scalar;
  }
  else if (trans == false && scalar == 1.0)
  {
    for (int i = 0; i < M; ++i)
      for (int j = 0; j < N; ++j)
        result[i * M + j] = A[i + j * LDA];
  }
  ...
  else if (trans == true &&  scalar != 1.0)
  {
    for (int i = 0; i < M; ++i)
      for (int j = 0; j < N; ++j)
        result[i * M + j] = A[i * LDA + j] * scalar;
  }
  else
  {
    for (int i = 0; i < M; ++i)
      for (int j = 0; j < N; ++j)
        result[i * M + j] = A[i * LDA + j];
  }
...
/*  
  ...
  */
  ...
  // set variable stack arrays
  float stackA[M * K], stackB[K * N], stackC[M * N];

  // copy fortran arguments to C
  auxA = copy(A, M, K, LDA, stackA, transpose_A, alpha, true); // alpha * op(A)
  auxB = copy(B, K, N, LDB, stackB, transpose_B, 1.0, true); // op(B)
  auxC = copy(C, M, N, LDC, stackC, false, beta, false); // beta * C
  ...
  tiled_matmul_auto(
    M, N, K,
    (elem_t*)auxA, (elem_t*)auxB, auxC, result,
    K, N, N, N,
    MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
    NO_ACTIVATION, ACC_SCALE_IDENTITY, 0, false,
    false, false,
            ...
            float* Y, int* _incY)
{
  ...
  // set variable stack arrays
  float stackX[M], stackY[M], stackA[M * N];
  ...
  auxA = copy(A, M, N, LDA, stackA, transpose_A, 1.0, true);
