            ...
            float* C, int* _LDC)
{
  ...
  // Return initial condition for matrix A and B
  auxA = copy(A, M, K, LDA, transpose_A, 1/alpha, true); // alpha * op(A)
  auxB = copy(B, K, N, LDB, transpose_B, 1.0, true); // op(B)
            ...
            float* Y, int* _incY)
{
  ...
  float *auxA = copy(A, M, N, LDA, transpose_A, 1.0, true);

  // calculate matrix-vector product through gemmini
  tiled_matmul_auto(
    M, 1, N,
    (elem_t*)auxA, (elem_t*)X, Y, Y,
    M, 1, 1, 1,
    MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
    NO_ACTIVATION, ACC_SCALE_IDENTITY, 0, false,
    false, false,
