...
# define POLYBENCH_CACHE_SIZE_KB 512 //32770
#endif
...
/* Timer code (RDTSC). */
unsigned long int polybench_c_start, polybench_c_end;
...
#ifdef POLYBENCH_RISCV32
  __asm__ volatile ("rdcycle %0" : "=r"(cycles_lo));
  ret = (unsigned long int)cycles_lo;
  //printf ("%lu\n", ret);
  //ret = read_cycle();
#else // X86
  __asm__ volatile ("RDTSC" : "=a" (cycles_lo), "=d" (cycles_hi));
  ret = (unsigned long int)cycles_hi << 32 | cycles_lo;
#endif
#endif
