!******************************************************************************
!
!  SGEMM.F90: This file is part of the PolyBench/Fortran 1.0 test suite.
! 
...
!     Deallocation of Arrays 
      ...
      contains
        ...
        do i = 1, ni
          do j = 1, nj
            c(j, i) = i + ((DBLE(i - 1) * DBLE(j - 1))) / DBLE(ni)
          end do
        end do
        do i = 1, ni
          do j = 1, nk
            a(j, i) = 2*j + ((DBLE(i - 1) * DBLE(j - 1))) / DBLE(ni)
          end do
        end do
        do i = 1, nk
          do j = 1, nj
            b(j, i) = i + ((DBLE(i - 1) * DBLE(j - 1))) / DBLE(ni)
          end do
        end do
        end subroutine
        ...
        do i = 1, ni
          do j = 1, nj
            call printf_num(c(j,i))
            !if (mod((i  * ni) + j , nj) == 0) then
            !  call printf_line()
            !end if
          end do
          call printf_line()
